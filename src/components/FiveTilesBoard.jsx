...
import { getFiveTilesSquares, useFiveTilesSquaresDispatch, setFiveTilesSquares } from "../context/GameContexts/FiveTilesSquareContext";
...
import { checkFiveTilesBoardWinner } from "../Utils/Utils";
import { getTimer, setTimer, useTimerDispatch, stopTimer } from "../context/GameContexts/TimerContext";

function FiveTilesBoard() {
  const [fiveTilessquares, dispatchFiveTilesSquares] = [getFiveTilesSquares(), useFiveTilesSquaresDispatch()];
  ...
  const squareRows = new Array(5).fill(new Array(5).fill(null));
  let squareIndex = 0;

  const makeMove = function (squareIndex) {
    if (!fiveTilessquares[squareIndex] && !winner && (timer.timerEnabled ? timer.timerStatus === "running" : timer.timerStatus !== "running")) {
      // set next move
      setNextMove(dispatchNextMove, nextMove);
      // set squares
      const upDatedSquares = setFiveTilesSquares(dispatchFiveTilesSquares, nextMove, squareIndex, fiveTilessquares);
  ...
  useEffect(() => {
    ...
    const gotWinner = checkFiveTilesBoardWinner(fiveTilessquares);
    ...
    if (gotWinner) {
      ...
      // no winner found
      let isLastMove = true;
      for (let i = 0; i < fiveTilessquares.length; i++) {
        // if this is not the last move
        if (!fiveTilessquares[i]) {
          isLastMove = false;
          break;
        }
      }
